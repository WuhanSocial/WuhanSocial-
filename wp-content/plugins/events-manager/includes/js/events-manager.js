	...
	if( !em_maps_loaded ){
		...
		if ( jQuery('script#google-maps').length == 0 && ( typeof google !== 'object' || typeof google.maps !== 'object' ) ){ 
			...
			if( typeof EM.google_maps_api !== 'undefined' ){
				...
				script.src = proto + '//maps.google.cn/maps/api/js?v=3.24&libraries=places&callback=em_maps&key='+EM.google_maps_api;

			}else{

				script.src = proto + '//maps.google.cn/maps/api/js?v=3.24&libraries=places&callback=em_maps';
...
function em_maps() {
	...
	if( jQuery('select#location-select-id, input#location-address').length > 0 ){
		...
		jQuery('#em-location-reset a').click( function(){
			jQuery('#location-latitude').val(30.579845);
			jQuery('#location-longitude').val(114.290725);
			marker.setDraggable(true);
